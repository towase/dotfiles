...
description: |
  ...
  `categories/<name>.md` に切り出されており、そのファイルをそのまま sub-agent に渡す。集約後は findings を優先順位順に提示し、どれを PR の inline
  コメントとして投稿するかユーザーに判断を仰ぎ、その結果に従って PR にコメント投稿する（または却下）。
...
以下をまとめてユーザーに提示して終了する:

1. Phase 4 の集約レポート（全 findings）
2. Phase 5 の投稿判断結果
   - 投稿対象に選ばれた finding ID 一覧
   - 却下された finding ID 一覧
3. Phase 6 の投稿結果（実行された場合のみ）
   - 成功した finding ID と review URL
   - 失敗した finding ID とエラー内容
   - フォールバック投稿（PR 全体コメント）になった finding ID

**本スキルの責務は「レビュー実行 → 投稿判断 → PR コメント投稿」まで**。
