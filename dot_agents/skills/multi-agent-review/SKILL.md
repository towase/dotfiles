...
description: |
  ...
  `/tmp/multi-agent-review-*.md` に Markdown レポートとして書き出し、コンソールには要点のみを提示する。
...
集約結果は `/tmp/multi-agent-review-*.md` に Markdown レポートとして出力する。コンソールには
...
ファイルパスは `/tmp/multi-agent-review-{YYYYMMDD-HHMMSS}-{identifier}.md`。
...
**レポート**: `/tmp/multi-agent-review-{...}.md`
...
| `/tmp` への書き込みに失敗 | カレントディレクトリ直下に `multi-agent-review-{...}.md` を出力するフォールバックに切り替え、コンソールに代替パスを案内。それも失敗した場合のみコンソールにレポート全文を出してユーザーに保存を促す |
